...
function Procedure({
  ...
  useEffect(() => {
    if (droppedProcedure !== id.toString()) {
      if (blockRef.current) blockRef.current.style.zIndex = '1';
      setIsDropped(false);
    }
  }, [droppedProcedure]);
  ...
  function onBookHandler(): void {
    const procedure: BookingProcedureProps = {
      ...
      duration:
        ...
        mastersData?.treatments.find((treatment) => treatment.procedureName)!.options[0],
    };
    ...
    <>
      <div ref={blockRef} className={classes.procedureBlock}>
        ...
        <div className={classes.lowLevel}>
          ...
          <SlideDownButton classNames={classes.slideDownButton} isDropped={isDropped} toggleDropdown={toggleDropdown}>
            More about
          </SlideDownButton>
        </div>
      </div>
